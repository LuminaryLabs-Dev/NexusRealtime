...
- ProtoKits local branch `0.0.2` at `82c879f48d9a35db520015a7b219da8c349d7fd9`; fetched `origin/0.0.2` advanced during this run to `3d42b725dd166274ff6c5452beef86ef17863852`; ahead/behind `0/4`; package metadata `@luminarylabs/nexusrealtime-protokits@0.0.2`.
...
- ProtoKits local: `npm run check` -> failed after checking 370 JavaScript modules; `smoke:imports` reported 110 failures rooted in `ERR_MODULE_NOT_FOUND` for package `nexusrealtime` from `protokits/nexus-dsk-adapter/index.js`.
...
- Local and fetched ProtoKits package metadata now both report `@luminarylabs/nexusrealtime-protokits@0.0.2`, but neither local nor fetched validation has a package/workspace/CDN/link model that resolves package `nexusrealtime`.
...
- Playwright snapshot showed heading `DSK first-wave proof`, description text, and `Booting...`; console/network output showed 404s for `NexusRealtime/src/index.js`, ProtoKits `domain-foundation`, and ProtoKits `domain-service-kits`.
