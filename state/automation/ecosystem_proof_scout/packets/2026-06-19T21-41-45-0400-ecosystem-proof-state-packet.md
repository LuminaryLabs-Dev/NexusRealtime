...
- Fetched ProtoKits `origin/0.0.2` is 26 commits ahead of local and now includes `tests/universal-game-domain-kits.test.mjs` in aggregate validation, but standalone fetched-ref validation still fails before tests because package `nexusrealtime` is not resolvable.
