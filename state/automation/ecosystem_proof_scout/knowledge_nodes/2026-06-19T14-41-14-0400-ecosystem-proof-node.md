...
- statement: Core and local available-checkout DSK proof remain green, but the current review blockers are still module-source gates: ProtoKits fetched-ref proof cannot resolve package `nexusrealtime`, Experiments fetched-ref targeted proof cannot resolve sibling `NexusRealtime`, and public browser proof remains stuck at `Booting...` on deployed sibling module 404s.
