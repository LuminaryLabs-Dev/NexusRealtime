...
Result: partial; core remains latest-release aligned and smoke-green, ProtoKits remains latest-ref aligned but cannot resolve package `nexusrealtime`, Experiments fetched aggregate still passes while targeted DSK proof fails on sibling paths, and the public proof route remains visible but stuck at `Booting...`.
...
- ProtoKits disposable fetched-ref extraction at `origin/0.0.2` `3d42b725dd166274ff6c5452beef86ef17863852`: `npm run check` -> failed after 370 syntax-checked modules with 110 import-smoke failures rooted in missing package `nexusrealtime`; targeted `node tests/dsk-first-wave.test.mjs` failed with the same missing package.
...
- `protokits/nexus-dsk-adapter/index.js` imports `nexusrealtime` as a bare package.
...
- Local and fetched ProtoKits package metadata both report `@luminarylabs/nexusrealtime-protokits@0.0.2`, but neither path declares or models package/workspace/CDN/link resolution for `nexusrealtime`.
